...
def prepare_data_gf_if(tensor_list, labels, test_size, batch_size, random_state):
...
# Train - Test function
def train_test_gf_if(model, train_loader, test_loader, criterion, optimizer, epochs):
...
for lr in learn_rates:
    ...
    train_loader, test_loader = prepare_data_gf_if([marbert_tensor, text_aux_tensor, user_profile_tensor, user_cred_tensor, sentiment_tensor], labels_tensor, test_size=test_size, batch_size=batch, random_state=seed_value)

    res = train_test_gf_if(model, train_loader, test_loader, criterion, optimizer, epochs=epochs)
    
    printable_results = {'Model': 'GF-IF', 'Hyperparams': lr, 'TP': res[0], 'TN': res[1], 'FP': res[2], 'FN': res[3],'Accuracy': format_f(res[4]), 'Precision': format_f(res[5]), 'Recall': format_f(res[6]), 'F1': format_f(res[7]), 'AUC': format_f(res[8])}
